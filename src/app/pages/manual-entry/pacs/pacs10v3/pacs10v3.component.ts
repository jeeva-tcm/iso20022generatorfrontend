...
export class Pacs10v3Component implements OnInit, OnDestroy {
    ...
    private buildForm() {
        ...
        const c: any = {
            ...
            bizSvc: ['swift.cbprplus.04', [Validators.required, Validators.maxLength(35)]],
            ...
            instrId:['INSTR-PAC010-001', [Validators.required, Validators.maxLength(16), Validators.pattern(/^[a-zA-Z0-9\/\-\?:\(\)\.,\+' ]*$/)]],
        ...
        prefixes.forEach(p => {
            ...
            if (!c[p + 'AcctType']) c[p + 'AcctType'] = [acctDefaults.AcctIBAN ? 'IBAN' : 'none'];
            if (!c[p + 'AcctIBAN']) {
    ...
    generateXml() {
        ...
        const srMsg = this.srVersionSvc.getMessageRules('pacs010MarginCollection');
        appHdr += `\t\t<BizMsgIdr>${this.e(v.bizMsgId)}</BizMsgIdr>\n\t\t<MsgDefIdr>${this.e(v.msgDefIdr || srMsg.msgDefIdr)}</MsgDefIdr>\n\t\t<BizSvc>${this.e(v.bizSvc || srMsg.bizSvc)}</BizSvc>\n`;
...
${appHdr}\t</AppHdr>
\t<Document xmlns="${srMsg.namespace}">
\t\t<FIDrctDbt>
\t\t\t<GrpHdr>\t<MsgId>${this.e(v.msgId)}</MsgId>\t<CreDtTm>${creDtTm}</CreDtTm>\t<NbOfTxs>${this.e(v.nbOfTxs || '1')}</NbOfTxs>\t</GrpHdr>
\t\t\t<CdtInstr>
...
\t\t\t\t</DrctDbtTxInf>\t\t\t</CdtInstr>\t\t</FIDrctDbt>\t</Document></BusMsgEnvlp>`;
    ...
    private formatAcctDetails(v: any, p: string, tabs: number) {
        const acctType = v[p + 'AcctType'] || 'none';
        if (acctType === 'IBAN' && v[p + 'AcctIBAN']) return this.el('IBAN', v[p + 'AcctIBAN'], tabs);
        if (acctType === 'Othr') {
    ...
    validateMessage() {
        ...
        this.http.post(this.config.getApiUrl('/validate'), {
            ...
            mode: 'Full 1-3',
            message_type: this.srVersionSvc.getMsgDefIdr('pacs010MarginCollection'),
