...
export class Pacs10Component implements OnInit, OnDestroy {
    ...
    generateXml() {
        ...
        const srMsg = this.srVersionSvc.getMessageRules('pacs010Interbank');
...
\t</Document>
    ...
    agt(tag: string, prefix: string, v: any, indent = 3, onlyBic = false) {
        ...
        if (!onlyBic && !bic) {
            ...
            // CBPR+ co-presence rule: Nm and PstlAdr must always appear together or both absent.
            if (name && addr) {
            ...
            // name-only (no address) is intentionally suppressed to satisfy the co-presence rule.
        }
    ...
    validateMessage() {
        ...
        this.http.post(this.config.getApiUrl('/validate'), {
            ...
            mode: 'Full 1-3',
            message_type: this.srVersionSvc.getMsgDefIdr('pacs010Interbank'),
    ...
    /**
     * Called whenever the SR version changes.
     * Patches version-driven form fields without clearing user data.
     */
    private applyVersionDefaults(): void {
        const cfg = this.srVersionSvc.getMessageRules('pacs010Interbank');
        this.form.patchValue({
