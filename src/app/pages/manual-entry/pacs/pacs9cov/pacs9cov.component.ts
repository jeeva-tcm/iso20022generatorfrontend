...
export class Pacs9CovComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.verDelta = this.versionDelta.bind('pacs009cov');
...
<BusMsgEnvlp xmlns="urn:swift:xsd:envelope">
	<AppHdr xmlns="urn:iso:std:iso:20022:tech:xsd:head.001.001.02">
		...
		<MsgDefIdr>${this.srVersion.getMsgDefIdr('pacs009Cov')}</MsgDefIdr>
		<BizSvc>${this.srVersion.getBizSvc('pacs009Cov')}</BizSvc>
		<CreDt>${creDtTm}</CreDt>${v.appHdrPriority?.trim() ? `\n\t\t<Prty>${v.appHdrPriority}</Prty>` : ''}
	</AppHdr>
	<Document xmlns="${this.srVersion.getNamespace('pacs009Cov')}">
		<FICdtTrf>
			<GrpHdr>
...
${tx}\t\t\t</CdtTrfTxInf>
    ...
    agt(tag: string, prefix: string, v: any, indent = 4) {
        ...
        // dbtrFi, cdtrFi, covDbtrAgt, covCdtrAgt) BICFI + Nm + PstlAdr are permitted
        // together, so let user-entered Name/Address propagate to the XML.
