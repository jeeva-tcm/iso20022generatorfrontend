...
export class Pacs9Component implements OnInit {
    ...
    generateXml() {
        ...
        if (v.svcLvlCd?.trim()) pmtTpXml += this.tag('SvcLvl', this.el('Cd', v.svcLvlCd, 6), 5);
        else if (v.svcLvlPrtry?.trim()) pmtTpXml += this.tag('SvcLvl', this.el('Prtry', v.svcLvlPrtry, 6), 5);
        if (v.lclInstrmCd?.trim()) pmtTpXml += this.tag('LclInstrm', this.el('Cd', v.lclInstrmCd, 6), 5);
        else if (v.lclInstrmPrtry?.trim()) pmtTpXml += this.tag('LclInstrm', this.el('Prtry', v.lclInstrmPrtry, 6), 5);
        if (v.ctgyPurpCd?.trim()) pmtTpXml += this.tag('CtgyPurp', this.el('Cd', v.ctgyPurpCd, 6), 5);
        else if (v.ctgyPurpPrtry?.trim()) pmtTpXml += this.tag('CtgyPurp', this.el('Prtry', v.ctgyPurpPrtry, 6), 5);
        if (pmtTpXml) tx += this.tag('PmtTpInf', pmtTpXml, 4);
        tx += `\t\t\t\t<IntrBkSttlmAmt Ccy="${this.e(v.currency)}">${this.e(v.amount)}</IntrBkSttlmAmt>\n`;
