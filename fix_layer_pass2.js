...
const directory = "C:\\Users\\HP\\Desktop\\iso final\\iso20022generatorfrontend\\src\\app";
...
const pattern = /\s*isLayerPass\(k: string\) \{[\s\S]*?isLayerWarn\(k: string\) \{[\s\S]*?\n\s*\}/;
...
function walk(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            if (content.includes('isLayerPass(k: string) {') && content.includes("includes('?')")) {
