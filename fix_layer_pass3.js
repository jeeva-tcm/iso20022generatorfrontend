...
const new_pass = `isLayerPass(k: string) {
...
const new_fail = `isLayerFail(k: string) {
    const s = (this.getLayerStatus(k) || '').toUpperCase();
    return s.includes('FAIL') || s.includes('ERROR') || s.includes('❌') || s.includes('?');
  }`;

const new_warn = `isLayerWarn(k: string) {
    const s = (this.getLayerStatus(k) || '').toUpperCase();
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            if (content.includes('isLayerPass(k: string) {')) {
                // Safely replace each function individually
                let new_content = content.replace(/isLayerPass\(k: string\) \{[\s\S]*?\n  \}/, new_pass);
                new_content = new_content.replace(/isLayerFail\(k: string\) \{[\s\S]*?\n  \}/, new_fail);
                new_content = new_content.replace(/isLayerWarn\(k: string\) \{[\s\S]*?\n  \}/, new_warn);
